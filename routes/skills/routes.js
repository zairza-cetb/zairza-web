...
isMentor = function (req, res, next) {
  if (req.method == "POST") {
    ...
    return res
      .status(404)
      .send({ status: "fail", message: "Method not supported" });
  }
};
...
router.get("/mentor-dashboard", isMentor, function (req, res, next) {
  DomainRegistrations.aggregate(
    ...
    function (err, result) {
      ...
      res.render("pages/dashboard/mentorsdashboard", {
        user: req.user,
        layout: "pages/base",
        userData: result,
        domains: req.domains,
        selectedDomain: req.query.domainId,
      });
    }
  );
...
router.get("/user-dashboard", async function (req, res, next) {
  const now = Date.now();
  isRegistered = await DomainRegistrations.findOne({
    user: req.user._id,
  }).exec();
...
apiRouter.post("/register", function (req, res, next) {
  ...
  if (now >= config.registrationEnd) {
    return res.status(400).send({
      status: "fail",
      message: "Sorry! The registration deadline has passed",
    });
  }
  ...
  Domains.findById(req.body.domainId, function (err, domain) {
    ...
    if (!domain) {
      return res
        .status(500)
        .send({ status: "fail", message: "Domain not found" });
    }
...
apiRouter.post("/user-submit", function (req, res, next) {
  ...
  if (!weekNo || 1 > weekNo || weekNo > config.maxWeekNos)
    return res
      .status(500)
      .send({ status: "fail", message: "Invalid week number" });
